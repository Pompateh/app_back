...
const saveProject = async () => {
  try {
    // Clean blocks before saving (remove id)
    const cleanedBlocks = formData.blocks.map((block) => {
      const { id, ...rest } = block as any;
      return rest;
    });

    // Team doesn't have id field based on your type, so no need to destructure id
    const cleanedTeam = formData.team.map((member) => ({ ...member }));
    ...
    if (formData.id) {
      await axios.put(`${API}/projects/${formData.id}`, payload, config);
    } else {
