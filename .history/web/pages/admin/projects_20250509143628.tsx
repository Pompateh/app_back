...
const AdminProjects: React.FC = () => {
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      if (formData.id) {
        ...
        await axiosInstance.put(`/projects/${formData.id}`, updateData);
      } else {
        ...
        await axiosInstance.post('/projects', { ...newProjectData, slug: slug || '', type: type || '' });
      }
      await mutate();
      setShowForm(false);
    } catch (error: any) {
      ...
      if (error.response?.status === 401) {
        ...
        alert(`Save failed: ${error.response.data.message || 'Invalid request data.'}`);
      } else {
  ...
  {formData.thumbnail && <img src={formData.thumbnail} className="h-32 w-full object-cover mb-2" />} {/* Increased size */}
