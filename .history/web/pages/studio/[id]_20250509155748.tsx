...
const fetcher = (url: string) =>
  ...
  const formatSlogan = (s: string) =>
    s.split('\n').map((line, i) => (
      <span key={i} className="block leading-tight tracking-wide text-center" style={{ fontSize: '5.3rem' }}>
        {line.split(' ').map((w, wi) =>
          <span key={wi} className={w === 'Powerhouse' ? 'italic' : ''}>{w} </span>
        )}
      </span>
    ));
      ...
      <section className="w-full py-28 bg-black text-white">
        <div className="max-w-screen-2xl mx-auto px-4 ">
          <div className="flex flex-col items-center">
            ...
            <h1 className="mb-6">{formatSlogan(studio.slogan)}</h1>
            <p className="text-3xl italic">{studio.tagline}</p>
          </div>
        </div>
      </section>
      ...
      {studio.fonts && (
        <section className="w-full pt-6 bg-gray-50">
          <div className="max-w-screen-2xl mx-auto px-4 ">
            ...
            <p className="mb-2">{studio.sections.fonts.preTitle}</p>
            <h3 className="text-2xl font-semibold mb-4">{studio.sections.fonts.title}</h3>
            <p className="mb-8" dangerouslySetInnerHTML={{ __html: studio.sections.fonts.description }} /><p className="mb-8" dangerouslySetInnerHTML={{ __html: studio.sections.fonts.description }} />
...
<button className="w-full py-3 bg-yellow-400 font-bold hover:bg-yellow-500 mt-6">
          {studio.sections.fonts.buttonText}
        </button>
</div>
...
{studio.artworks && (
  <section className="w-full pt-12 bg-white">
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <p className="mb-2">{studio.sections.artworks.preTitle}</p>
      <h3 className="text-3xl font-semibold mb-4" dangerouslySetInnerHTML={{ __html: studio.sections.artworks.title }} />
      <p className="mb-8">{studio.sections.artworks.description}</p>
...
<div className="border-wrapper">
<button className="w-full py-3 bg-yellow-400 font-bold hover:bg-yellow-500 mt-6">
          {studio.sections.artworks.buttonText}
        </button>
</div>

<section
      className="py-32"
      style={{ backgroundImage: `url(${studio.sections.newsletter.backgroundImage})` }}
    >
  <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center">
    ...
    <div className="w-full md:w-1/2 flex justify-center items-center">
      ...
      <h2 className="text-3xl">
            {studio.sections.newsletter.title.split('\n').map((l,i) => <span key={i} className="block">{l}</span>)}
          </h2>
        <input
  type="text"
