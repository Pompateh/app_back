    ...
    switch (block.type) {
      case 'text':
        return (
          <>
                        {block.layout === 'right' && (
              <div className="border-t-2 border-l-2 border-b-1 border-[#999380] md:col-start-1 md:col-span-1 "></div>
            )}
            <div
              key={idx}
              className={`border-t-2 border-r-2 border-l-2  border-[#999380] p-4 prose lg:prose-lg ${
                block.layout === 'left' ? 'md:col-start-1 md:col-span-1' : 'md:col-start-2 md:col-span-1'
              }`}
            >
              <div dangerouslySetInnerHTML={{ __html: block.text }} />
            </div>
            {block.layout === 'left' && <div className="border-2 border-[#999380] md:col-start-2 md:col-span-1"></div>}
          </>
        );
      case 'full_image': {
        ...
        return (
          <div key={idx} className="grid grid-cols-2 grid-rows-2 gap-0 md:col-span-2">
            {block.data.items.map((item, i) => {
              ...
              if (i === 2) {
                cellClass = 'row-span-2 border-l-2 border-r-2 row-start-1 col-start-2';
              }
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const relatedAll: ProjectDetail[] = await fetch(`${API_BASE}/projects`).then((r) => r.json());
  const related = Array.isArray(relatedAll)
    ? relatedAll.filter((p) => p.slug !== params.slug)
    : [];
  return { props: { project, related }, revalidate: 60 };
};
